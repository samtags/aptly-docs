...
function ChatComponent() {
  const { message, metadata } = useStream('chat-room');
...
Automatically subscribes to a topic and provides the latest message.
...
function MyComponent() {
  const { message, metadata, streaming, subscribe, unsubscribe } = useStream('my-topic');
    ...
    <div>
      {streaming ? (
        <div>
          ...
          <p>Offset: {metadata?.offset}</p>
          <p>Timestamp: {metadata?.timestamp}</p>
          <p>Replay: {metadata?.replay ? 'Yes' : 'No'}</p>
        </div>
      ) : (
        <p>Not streaming</p>
      )}
...
| `metadata` | `object` | Message metadata (offset, timestamp, replay, channel) |
...
function MessageDisplay() {
  const { message, metadata, streaming } = useStream('notifications');
  ...
  return (
    <div>
      ...
      <small>Received at: {metadata?.timestamp}</small>
    </div>
  );
}
```
