...
<Note>
  **About the Origin Header:** The Origin header is required for CORS validation
  when using public API keys. When making requests from a browser, the Origin
  header is automatically sent by the browser and you don't need to manually
  specify it. For server-side requests (Node.js, serverless functions, cURL)
  using public keys, you must manually include the Origin header with one of
  your allowed website URLs. If you're using a server key, the Origin header is
  not required.
</Note>
